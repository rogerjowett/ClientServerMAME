...
struct NoPriorityType { char dummy[3]; };


/* macros for using the optional priority */
#define PRIORITY_VALID(x)		(sizeof(x) != sizeof(NoPriorityType))
...
/*
    Assumed input parameters or local variables:
        ...
        bitmap_t *priority - the priority bitmap (even if PRIORITY_TYPE is NoPriorityType, at least needs a dummy)
*/
