...
void am53cf96_init( running_machine *machine, const AM53CF96interface *aminterface )
{
	int i;
	...
	for (i = 0; i < aminterface->scsidevs->devs_present; i++)
	{
		SCSIAllocInstance( machine, aminterface->scsidevs->devices[i].scsiClass, &devices[aminterface->scsidevs->devices[i].scsiID], aminterface->scsidevs->devices[i].diskregion );
	}
...
void am53cf96_exit( const AM53CF96interface *aminterface )
{
	int i;

	for (i = 0; i < aminterface->scsidevs->devs_present; i++)
	{
		SCSIDeleteInstance( devices[aminterface->scsidevs->devices[i].scsiID] );
	}
}
