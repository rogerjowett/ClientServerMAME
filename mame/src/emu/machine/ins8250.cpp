...
typedef struct {
	const ins8250_interface *ins_interface;
	int	device_type;
...
#define COM_INT_PENDING_MODEM_STATUS_REGISTER 0x0008
 ...
 ins8250_t *get_safe_token_ins8250(running_device *device)
{
	assert( device != NULL );
...
{
	ins8250_t	*ins8250 = get_safe_token_ins8250(device);
...
static void ins8250_update_interrupt(running_device *device)
{
	...
	if (ins8250->ins_interface->interrupt)
		ins8250->ins_interface->interrupt(device, state);
}
...
WRITE8_DEVICE_HANDLER( ins8250_w )
{
	...
	switch (offset)
	{
		case 0:
			if (ins8250->lcr & 0x80)
			{
				...
				COM_LOG(1,"COM_dll_w",("COM \"%s\" $%02x: [$%04x = %d baud]\n", device->tag(),
					 data, tmp, (tmp)?(int)(ins8250->ins_interface->clockin/16/tmp):0));
			}
			else
			{
				ins8250->thr = data;
				...
				if ( ins8250->ins_interface->transmit )
					ins8250->ins_interface->transmit(device, ins8250->thr);
		...
		case 4:
			if ( ( ins8250->mcr & 0x1f ) != ( data & 0x1f ) )
			{
				...
				if (ins8250->ins_interface->handshake_out)
					ins8250->ins_interface->handshake_out(device,data);
	...
	if (ins8250->ins_interface->refresh_connected)
		ins8250->ins_interface->refresh_connected(device);
}
...
static void common_start( running_device *device, int device_type )
{
	...
	ins8250->ins_interface = (const ins8250_interface*)device->baseconfig().static_config();
