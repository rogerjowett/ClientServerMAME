...
void lsi53c810_init(running_machine *machine, const LSI53C810interface *lsinterface)
{
	int i;
	...
	for (i = 0; i < lsinterface->scsidevs->devs_present; i++)
	{
		SCSIAllocInstance( machine, lsinterface->scsidevs->devices[i].scsiClass, &devices[lsinterface->scsidevs->devices[i].scsiID], lsinterface->scsidevs->devices[i].diskregion );
	}
}

void lsi53c810_exit(const LSI53C810interface *lsinterface)
{
	int i;

	for (i = 0; i < lsinterface->scsidevs->devs_present; i++)
	{
		SCSIDeleteInstance( devices[lsinterface->scsidevs->devices[i].scsiID] );
	}
}
