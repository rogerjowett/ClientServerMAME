...
void wd33c93_init( running_machine *machine, const WD33C93interface *wdinterface )
{
	int i;
	...
	for (i = 0; i < wdinterface->scsidevs->devs_present; i++)
	{
		SCSIAllocInstance( machine, wdinterface->scsidevs->devices[i].scsiClass, &devices[wdinterface->scsidevs->devices[i].scsiID], wdinterface->scsidevs->devices[i].diskregion );
	}
...
void wd33c93_exit( const WD33C93interface *wdinterface )
{
	int i;

	for (i = 0; i < wdinterface->scsidevs->devs_present; i++)
	{
		SCSIDeleteInstance( devices[wdinterface->scsidevs->devices[i].scsiID] );
	}
}
