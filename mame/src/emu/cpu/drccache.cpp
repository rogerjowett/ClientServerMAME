...
{
	/* core parameters */
	drccodeptr			near_ptr;				/* pointer to the near_ptr part of the cache */
...
drccache *drccache_alloc(size_t bytes)
{
	...
	cache.near_ptr = (drccodeptr)osd_alloc_executable(bytes);
	cache.neartop = cache.near_ptr;
	cache.base = cache.near_ptr + NEAR_CACHE_SIZE;
	cache.top = cache.base;
	cache.end = cache.near_ptr + bytes;
	cache.size = bytes;
...
void drccache_free(drccache *cache)
{
	...
	osd_free_executable(cache->near_ptr, cache->size);
}
...
{
	return ((const drccodeptr)ptr >= cache->near_ptr && (const drccodeptr)ptr < cache->near_ptr + cache->size);
}
...
{
	return ((const drccodeptr)ptr >= cache->near_ptr && (const drccodeptr)ptr < cache->neartop);
}
...
void drccache_memory_free(drccache *cache, void *memory, size_t bytes)
{
	...
	assert(((drccodeptr)memory >= cache->near_ptr && (drccodeptr)memory < cache->base) || ((drccodeptr)memory >= cache->end && (drccodeptr)memory < cache->near_ptr + cache->size));
