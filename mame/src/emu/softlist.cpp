...
static void add_software_part(software_list *swlist, const char *name, const char *sl_interface)
{
	software_part *part = &swlist->softinfo->partdata[swlist->current_part_entry];

	part->name = name;
	part->interface_ = sl_interface;
			...
			else if ( !strcmp(tagname, "part" ) )
			{
				...
				if ( str_name && str_interface )
				{
					if ( swlist->softinfo )
					{
						...
						char *sl_interface = (char *)pool_malloc_lib(swlist->pool, ( strlen( str_interface ) + 1 ) * sizeof(char) );

						if ( !name || !sl_interface )
							return;

						strcpy( name, str_name );
						strcpy( sl_interface, str_interface );

						add_software_part( swlist, name, sl_interface );
...
software_part *software_find_part(software_info *sw, const char *partname, const char *sl_interface)
{
	software_part *part = sw ? sw->partdata : NULL;

	 /* If neither partname nor sl_interface supplied, then we just return the first entry */
	if ( partname || sl_interface )
	{
		while( part && part->name )
		{
			if ( partname )
			{
				if ( !strcmp(partname, part->name ) )
				{
					if ( sl_interface )
					{
						if ( !strcmp(sl_interface, part->interface_) )
						{
							break;
						}
					}
			...
			{
				/* No specific partname supplied, find the first match based on sl_interface */
				if ( sl_interface )
				{
					if ( !strcmp(sl_interface, part->interface_) )
					{
...
bool load_software_part(device_image_interface *image, const char *path, software_info **sw_info, software_part **sw_part, char **full_sw_name)
{
	...
	*sw_part = NULL;
	...
	const char *sl_interface = image->image_config().image_interface();
	...
	if ( swlist_name )
	{
		...
		if ( software_list_ptr )
		{
			...
			{
				software_part_ptr = software_find_part( software_info_ptr, swpart, sl_interface );
			}
		}
	}
	else
...
static void ui_mess_menu_populate_software_entries(running_machine *machine, ui_menu *menu, char *list_name, device_image_interface* image)
{
	...
	if (list)
	{
		for (software_info *swinfo = software_list_find(list, "*", NULL); swinfo != NULL; swinfo = software_list_find(list, "*", swinfo))
		{
			...
			entry->sl_interface = ui_menu_pool_strdup(menu, part->interface_);
			if (strcmp(sl_interface,part->interface_)==0) {
...
void ui_mess_menu_software_list(running_machine *machine, ui_menu *menu, void *parameter, void *state)
{
	...
	if (event != NULL && event->iptkey == IPT_UI_SELECT && event->itemref != NULL)
	{
		...
		if (image != NULL)
			...
			popmessage("No matching device found for sl_interface '%s'!", entry->sl_interface);
	}
}
