...
int mame_execute(core_options *options)
{
	...
	while (error == MAMERR_NONE && !exit_pending)
	{
		...
		try
		{
			...
			if(netServer) netServer->initializeConnection();
			...
			while ((!mame->hard_reset_pending && !mame->exit_pending) || mame->saveload_pending_file.len() != 0)
			{
                ...
                */
				...
				if(first)
				{
					...
					if(netClient) 
					{
						...
						//handle_load(machine);
						netClient->initializeConnection(
                ...
                */
				...
				if(lastSyncTime+CLOCKS_PER_SEC/60 < clock() || clientInputDatabase.size()==0)
				{
				    lastSyncTime = clock();
				    if(netClient && netClient->hasConnection())
				    {
                        while(true)
				        {
                            ...
                            bool gotsync=netClient->syncAndUpdate();
        					if(gotsync)
                            {
