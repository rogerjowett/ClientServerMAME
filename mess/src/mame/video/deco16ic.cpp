...
{
	deco16ic_state *deco16ic = get_safe_token_deco16ic(device);
...
*****************************************************************************/
 ...
 deco16ic_state *get_safe_token_deco16ic( running_device *device )
{
	assert(device != NULL);
 ...
 const deco16ic_interface *get_interface_deco16ic( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == DECO16IC));
...
void deco16ic_pf34_set_gfxbank( running_device *device, int small_deco, int big_deco )
{
	...
	if (deco16ic->pf34_last_big != big_deco)
	{
		...
		deco16ic->pf34_last_big = big_deco;
	}
	deco16ic->pf34_16x16_gfx_bank = big_deco;
}
...
{
	deco16ic_state *deco16ic = get_safe_token_deco16ic(device);
	const deco16ic_interface *intf = get_interface_deco16ic(device);
	int width, height;
