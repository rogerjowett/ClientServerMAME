...
*****************************************************************************/
 ...
 kaneko_pandora_state *get_safe_token_kan_pand( running_device *device )
{
	assert(device != NULL);
 ...
 const kaneko_pandora_interface *get_interface_kan_pand( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == KANEKO_PANDORA));
...
{
	kaneko_pandora_state *pandora = get_safe_token_kan_pand(device);
	pandora->bg_pen = pen;
}
...
{
	kaneko_pandora_state *pandora = get_safe_token_kan_pand(device);
	const kaneko_pandora_interface *intf = get_interface_kan_pand(device);
