...
*****************************************************************************/
 ...
 sega16sp_state *get_safe_token_sega16sp( running_device *device )
{
	assert(device != NULL);
 ...
 const sega16sp_interface *get_interface_sega16sp( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == SEGA16SP));
...
void segaic16_sprites_hangon_draw(running_machine *machine, running_device *device, bitmap_t *bitmap, const rectangle *cliprect)
{
	...
	sega16sp_state *sega16sp = get_safe_token_sega16sp(device);
...
void segaic16_sprites_draw(running_device *screen, bitmap_t *bitmap, const rectangle *cliprect, int which)
{
	...
	sega16sp = get_safe_token_sega16sp(device);
...
{
	sega16sp_state *sega16sp = get_safe_token_sega16sp(device);
	const sega16sp_interface *intf = get_interface_sega16sp(device);
	int i;
