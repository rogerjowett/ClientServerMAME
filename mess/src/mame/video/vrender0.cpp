 ...
 *****************************************************************************/
 ...
 vr0video_state *get_safe_token_vrender0( running_device *device )
{
	assert(device != NULL);
 ...
 const vr0video_interface *get_interface_vrender0( running_device *device )
{
	assert(device != NULL);
	assert(device->type() == VIDEO_VRENDER0);
...
{
	vr0video_state *vr0 = get_safe_token_vrender0(device);
...
{
	vr0video_state *vr0 = get_safe_token_vrender0(device);
	const vr0video_interface *intf = get_interface_vrender0(device);
