...
INLINE pc080sn_state *pc080sn_get_safe_token_taitoic( running_device *device )
{
	assert(device != NULL);
...
INLINE const pc080sn_interface *pc080sn_get_interface_taitoic( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == PC080SN));
...
{
	pc080sn_state *pc080sn = pc080sn_get_safe_token_taitoic(device);
	UINT16 code, attr;
...
{
	pc080sn_state *pc080sn = pc080sn_get_safe_token_taitoic(device);
	const pc080sn_interface *intf = pc080sn_get_interface_taitoic(device);
...
INLINE pc090oj_state *pc090oj_get_safe_token_taitoic( running_device *device )
{
	assert(device != NULL);
...
INLINE const pc090oj_interface *pc090oj_get_interface_taitoic( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == PC090OJ));
...
{
	pc090oj_state *pc090oj = pc090oj_get_safe_token_taitoic(device);
...
{
	pc090oj_state *pc090oj = pc090oj_get_safe_token_taitoic(device);
	const pc090oj_interface *intf = pc090oj_get_interface_taitoic(device);
...
INLINE tc0080vco_state *tc0080vco_get_safe_token_taitoic( running_device *device )
{
	assert(device != NULL);
...
INLINE const tc0080vco_interface *tc0080vco_get_interface_taitoic( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == TC0080VCO));
...
{
	tc0080vco_state *tc0080vco = tc0080vco_get_safe_token_taitoic(device);
	int color, tile;
...
{
	tc0080vco_state *tc0080vco = tc0080vco_get_safe_token_taitoic(device);
	const tc0080vco_interface *intf = tc0080vco_get_interface_taitoic(device);
...
INLINE tc0100scn_state *tc0100scn_get_safe_token_taitoic( running_device *device )
{
	assert(device != NULL);
...
INLINE const tc0100scn_interface *tc0100scn_get_interface_taitoic( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == TC0100SCN));
...
{
	tc0100scn_state *tc0100scn = tc0100scn_get_safe_token_taitoic(device);
	int code, attr;
...
{
	tc0100scn_state *tc0100scn = tc0100scn_get_safe_token_taitoic(device);
	const tc0100scn_interface *intf = tc0100scn_get_interface_taitoic(device);
	int xd, yd;
...
INLINE tc0280grd_state *tc0280grd_get_safe_token_taitoic( running_device *device )
{
	assert(device != NULL);
...
INLINE const tc0280grd_interface *tc0280grd_get_interface_taitoic( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == TC0280GRD) || (device->type() == TC0430GRW));
...
{
	tc0280grd_state *tc0280grd = tc0280grd_get_safe_token_taitoic(device);
...
{
	tc0280grd_state *tc0280grd = tc0280grd_get_safe_token_taitoic(device);
	const tc0280grd_interface *intf = tc0280grd_get_interface_taitoic(device);
...
INLINE tc0360pri_state *tc0360pri_get_safe_token_taitoic( running_device *device )
{
	assert(device != NULL);
...
{
	tc0360pri_state *tc0360pri = tc0360pri_get_safe_token_taitoic(device);
...
INLINE tc0480scp_state *tc0480scp_get_safe_token_taitoic( running_device *device )
{
	assert(device != NULL);
...
INLINE const tc0480scp_interface *tc0480scp_get_interface_taitoic( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == TC0480SCP));
...
{
	tc0480scp_state *tc0480scp = tc0480scp_get_safe_token_taitoic(device);
...
{
	tc0480scp_state *tc0480scp = tc0480scp_get_safe_token_taitoic(device);
	const tc0480scp_interface *intf = tc0480scp_get_interface_taitoic(device);
	int i, xd, yd;
...
INLINE tc0150rod_state *tc0150rod_get_safe_token_taitoic( running_device *device )
{
	assert(device != NULL);
...
INLINE const tc0150rod_interface *tc0150rod_get_interface_taitoic( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == TC0150ROD));
...
{
	tc0150rod_state *tc0150rod = tc0150rod_get_safe_token_taitoic(device);
...
{
	tc0150rod_state *tc0150rod = tc0150rod_get_safe_token_taitoic(device);
	const tc0150rod_interface *intf = tc0150rod_get_interface_taitoic(device);
...
INLINE tc0110pcr_state *tc0110pcr_get_safe_token_taitoic( running_device *device )
{
	assert(device != NULL);
...
INLINE const tc0110pcr_interface *tc0110pcr_get_interface_taitoic( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == TC0110PCR));
...
{
	tc0110pcr_state *tc0110pcr = tc0110pcr_get_safe_token_taitoic(device);
...
{
	tc0110pcr_state *tc0110pcr = tc0110pcr_get_safe_token_taitoic(device);
	const tc0110pcr_interface *intf = tc0110pcr_get_interface_taitoic(device);
...
INLINE tc0180vcu_state *tc0180vcu_get_safe_token_taitoic( running_device *device )
{
	assert(device != NULL);
...
INLINE const tc0180vcu_interface *tc0180vcu_get_interface_taitoic( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == TC0180VCU));
...
{
	tc0180vcu_state *tc0180vcu = tc0180vcu_get_safe_token_taitoic(device);
...
{
	tc0180vcu_state *tc0180vcu = tc0180vcu_get_safe_token_taitoic(device);
	const tc0180vcu_interface *intf = tc0180vcu_get_interface_taitoic(device);
