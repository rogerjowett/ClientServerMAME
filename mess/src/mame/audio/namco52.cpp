...
struct _namco_52xx_state
{
 ...
 namco_52xx_state *get_safe_token_namco52(running_device *device)
{
	assert(device != NULL);
...
{
	namco_52xx_state *state = get_safe_token_namco52((running_device *)ptr);
	state->latched_cmd = param;
}

static READ8_HANDLER( namco_52xx_K_r )
{
	namco_52xx_state *state = get_safe_token_namco52(space->cpu->owner());
...
{
	namco_52xx_state *state = get_safe_token_namco52(device);
