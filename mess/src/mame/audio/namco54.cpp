...
struct _namco_54xx_state
{
 ...
 namco_54xx_state *get_safe_token_namco54(running_device *device)
{
	assert(device != NULL);
...
{
	namco_54xx_state *state = get_safe_token_namco54((running_device *)ptr);
	state->latched_cmd = param;
}

static READ8_HANDLER( namco_54xx_K_r )
{
	namco_54xx_state *state = get_safe_token_namco54(space->cpu->owner());
...
{
	namco_54xx_state *state = get_safe_token_namco54(device);
