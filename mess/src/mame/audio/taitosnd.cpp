...
*****************************************************************************/
 ...
 tc0140syt_state *get_safe_token_taitosnd( running_device *device )
{
	assert(device != NULL);
 ...
 const tc0140syt_interface *get_interface_taitosnd( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == TC0140SYT));
...
{
	tc0140syt_state *tc0140syt = get_safe_token_taitosnd(device);
...
{
	tc0140syt_state *tc0140syt = get_safe_token_taitosnd(device);
	const tc0140syt_interface *intf = get_interface_taitosnd(device);
