...
static int turbofront_check(running_machine *machine, int tfc_small, int num)
{
	const UINT8 *sr = &machine->generic.spriteram.u8[0x800*tfc_small + 4*num];
	int x = sr[2] + ((sr[3] & 0x80) << 1);
	int y = sr[0] + ((sr[3] & 0x10) << 4);

	int dx = (x - (tfc_small ? tc16_posx : tc32_posx)) & 0x1ff;
	int dy = (y - (tfc_small ? tc16_posy : tc32_posy)) & 0x1ff;
...
static int turbofront_check8(running_machine *machine, int tfc_small, int num)
{
	return
		(turbofront_check(machine, tfc_small, num + 0) << 0) |
		(turbofront_check(machine, tfc_small, num + 1) << 1) |
		(turbofront_check(machine, tfc_small, num + 2) << 2) |
		(turbofront_check(machine, tfc_small, num + 3) << 3) |
		(turbofront_check(machine, tfc_small, num + 4) << 4) |
		(turbofront_check(machine, tfc_small, num + 5) << 5) |
		(turbofront_check(machine, tfc_small, num + 6) << 6) |
		(turbofront_check(machine, tfc_small, num + 7) << 7);
}
