...
-------------------------------------------------*/
 ...
 naomibd_state *get_safe_token_naomibd(running_device *device)
{
	assert(device != NULL);
...
int naomibd_interrupt_callback(running_device *device, naomibd_interrupt_func callback)
{
	...
	//naomibd_state *v = get_safe_token_naomibd(device);
...
{
	return get_safe_token_naomibd(device)->memory;
}
...
offs_t naomibd_get_dmaoffset(running_device *device)
{
	...
	#if NAOMIBD_PRINTF_PROTECTION
        printf("DMA source %08x, flags %x\n", get_safe_token_naomibd(device)->dma_offset, get_safe_token_naomibd(device)->dma_offset_flags);
	#endif

	// if the flag is cleared that lets the protection chip go,
	// we need to handle this specially.  but not on DIMM boards.
	if (!(get_safe_token_naomibd(device)->dma_offset_flags & NAOMIBD_FLAG_ADDRESS_SHUFFLE) && (get_safe_token_naomibd(device)->type == ROM_BOARD))
	{
		...
		{
			switch (get_safe_token_naomibd(device)->dma_offset)
			{
				...
				default:
					result = get_safe_token_naomibd(device)->dma_offset;
					break;
			}
