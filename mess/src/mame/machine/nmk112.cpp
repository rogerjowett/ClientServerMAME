...
*****************************************************************************/
 ...
 nmk112_state *get_safe_token_nmk112( running_device *device )
{
	assert(device != NULL);
 ...
 const nmk112_interface *get_interface_nmk112( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == NMK112));
...
{
	nmk112_state *nmk112 = get_safe_token_nmk112(device);
	int chip = (offset & 4) >> 2;
...
{
	nmk112_state *nmk112 = get_safe_token_nmk112(device);
	const nmk112_interface *intf = get_interface_nmk112(device);
