...
*****************************************************************************/
 ...
 namcoio_state *get_safe_token_namcoio( running_device *device )
{
	assert(device != NULL);
 ...
 const namcoio_interface *get_interface_namcoio( running_device *device )
{
	assert(device != NULL);
	assert(device->type() == NAMCO56XX || device->type() == NAMCO58XX || device->type() == NAMCO59XX);
...
{
	namcoio_state *namcoio = get_safe_token_namcoio(device);
	int val, toggled;
	int credit_add = 0;
	int credit_sub = 0;
	int button;
...
{
	namcoio_state *namcoio = get_safe_token_namcoio(device);
	const namcoio_interface *intf = get_interface_namcoio(device);
