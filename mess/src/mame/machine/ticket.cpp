...
struct _ticket_state
{
 ...
 ticket_state *get_safe_token_ticket(running_device *device)
{
	assert(device != NULL);
...
{
	ticket_state *state = get_safe_token_ticket((running_device *)ptr);
...
{
	ticket_state *state = get_safe_token_ticket(device);
