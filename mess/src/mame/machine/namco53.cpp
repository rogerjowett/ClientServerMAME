...
struct _namco_53xx_state
{
 ...
 namco_53xx_state *get_safe_token_namco53(running_device *device)
{
	assert(device != NULL);
...
{
	namco_53xx_state *state = get_safe_token_namco53(space->cpu->owner());
...
{
	namco_53xx_state *state = get_safe_token_namco53((running_device *)ptr);
	cpu_set_input_line(state->cpu, 0, CLEAR_LINE);
}

void namco_53xx_read_request(running_device *device)
{
	namco_53xx_state *state = get_safe_token_namco53(device);
