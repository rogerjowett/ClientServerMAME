...
***************************************************************************/
 ...
 vt_video_t *get_safe_token_vtvideo(running_device *device)
{
	assert(device != NULL);
 ...
 const vt_video_interface *get_interface_vtvideo(running_device *device)
{
	assert(device != NULL);
...
{
	vt_video_t *vt = get_safe_token_vtvideo(device);
	return vt->lba7;
}
...
{
	vt_video_t *vt = get_safe_token_vtvideo(device);
	const vt_video_interface *intf = get_interface_vtvideo(device);
