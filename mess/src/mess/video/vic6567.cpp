...
*****************************************************************************/
 ...
 vic2_state *get_safe_token_vic6567( running_device *device )
{
	assert(device != NULL);
 ...
 const vic2_interface *get_interface_vic6567( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == VIC2));
...
{
	vic2_state *vic2 = get_safe_token_vic6567(device);
	vic2->on = onoff;
}
