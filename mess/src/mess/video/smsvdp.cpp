...
*****************************************************************************/
 ...
 smsvdp_t *get_safe_token_smsvdp(running_device *device)
{
	assert(device != NULL);
 ...
 const smsvdp_interface *get_interface_smsvdp(running_device *device)
{
	assert(device != NULL);
	assert((device->type() == SMSVDP));
...
{
	smsvdp_t *smsvdp = get_safe_token_smsvdp(device);
...
UINT8 sms_vdp_area_brightness(running_device *device, int x, int y, int x_range, int y_range)
{
	...
	smsvdp = get_safe_token_smsvdp(device);
...
{
	smsvdp_t *smsvdp = get_safe_token_smsvdp(device);
	const smsvdp_interface *intf = get_interface_smsvdp(device);
