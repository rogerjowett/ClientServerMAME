...
***************************************************************************/
 ...
 upd7220_t *get_safe_token_upd7220(running_device *device)
{
	assert(device != NULL);
 ...
 const upd7220_interface *get_interface_upd7220(running_device *device)
{
	assert(device != NULL);
	assert((device->type() == UPD7220));
...
static TIMER_CALLBACK( vsync_tick )
{
	...
	upd7220_t *upd7220 = get_safe_token_upd7220(device);
...
{
	upd7220_t *upd7220 = get_safe_token_upd7220(device);
	const upd7220_interface *intf = get_interface_upd7220(device);
	int i;
