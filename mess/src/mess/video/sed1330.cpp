...
***************************************************************************/
 ...
 sed1330_t *get_safe_token_sed1330(running_device *device)
{
	assert(device != NULL);
	return (sed1330_t *)downcast<legacy_device_base *>(device)->token();
}

 const sed1330_interface *get_interface_sed1330(running_device *device)
{
	assert(device != NULL);
...
{
	sed1330_t *sed1330 = get_safe_token_sed1330(device);
...
{
	sed1330_t *sed1330 = get_safe_token_sed1330(device);
	const sed1330_interface *intf = get_interface_sed1330(device);
