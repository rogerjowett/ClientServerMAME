...
***************************************************************************/
 ...
 crt9007_t *get_safe_token_crt9007(running_device *device)
{
	assert(device != NULL);
	return (crt9007_t *)downcast<legacy_device_base *>(device)->token();
}

 const crt9007_interface *get_interface_crt9007(running_device *device)
{
	assert(device != NULL);
...
static TIMER_CALLBACK( vsync_tick )
{
	...
	crt9007_t *crt9007 = get_safe_token_crt9007(device);
...
static DEVICE_START( crt9007 )
{
	...
	const crt9007_interface *intf = get_interface_crt9007(device);
