...
***************************************************************************/
 ...
 cdp1861_t *get_safe_token_cdp1861(running_device *device)
{
	assert(device != NULL);
 ...
 const cdp1861_interface *get_interface_cdp1861(running_device *device)
{
	assert(device != NULL);
	assert((device->type() == CDP1861));
...
static TIMER_CALLBACK( cdp1861_int_tick )
{
	...
	cdp1861_t *cdp1861 = get_safe_token_cdp1861(device);
...
{
	cdp1861_t *cdp1861 = get_safe_token_cdp1861(device);
	const cdp1861_interface *intf = get_interface_cdp1861(device);
