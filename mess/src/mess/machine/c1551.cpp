...
***************************************************************************/
 ...
 c1551_t *get_safe_token_c1551(running_device *device)
{
	assert(device != NULL);
...
{
	c1551_t *c1551 = get_safe_token_c1551(timer.owner());
...
static TIMER_CALLBACK( bit_tick )
{
	...
	c1551_t *c1551 = get_safe_token_c1551(device);
	int byte = 0;
...
{
	c1551_t *c1551 = get_safe_token_c1551(image.device().owner());
...
static UINT8 c1551_port_r( running_device *device, UINT8 direction )
{
    ...
    */
	...
	c1551_t *c1551 = get_safe_token_c1551(device->owner());
	UINT8 data = 0;
