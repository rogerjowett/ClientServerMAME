...
***************************************************************************/
 ...
 i8214_t *get_safe_token_i8214(running_device *device)
{
	assert(device != NULL);
	return (i8214_t *)downcast<legacy_device_base *>(device)->token();
}

 const i8214_interface *get_interface_i8214(running_device *device)
{
	assert(device != NULL);
...
{
	i8214_t *i8214 = get_safe_token_i8214(device);
...
{
	i8214_t *i8214 = get_safe_token_i8214(device);
	const i8214_interface *intf = get_interface_i8214(device);
