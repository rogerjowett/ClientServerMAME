...
struct _scsibus_t
{
	...
	const SCSIBus_interface *scsi_interface;
...
static void scsi_out_line_change_now(running_device *device, UINT8 line, UINT8 state)
{
    ...
    if(bus->scsi_interface->line_change_cb!=NULL)
        bus->scsi_interface->line_change_cb(device,line,state);
}
...
{
    scsibus_t               *bus = get_token(device);
    const SCSIConfigTable   *scsidevs = bus->scsi_interface->scsidevs;
	int                     devno;
...
static DEVICE_START( scsibus )
{
    ...
    bus->scsi_interface = (const SCSIBus_interface*)device->baseconfig().static_config();
...
static DEVICE_STOP( scsibus )
{
	...
	for (devno = 0; devno < bus->scsi_interface->scsidevs->devs_present; devno++)
	{
