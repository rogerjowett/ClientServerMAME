...
*****************************************************************************/
 ...
 pc_lpt_state *get_safe_token_pc_lpt(running_device *device)
{
	assert(device != NULL);
...
{
	pc_lpt_state *lpt = get_safe_token_pc_lpt(device);
...
static WRITE_LINE_DEVICE_HANDLER( pc_lpt_ack_w )
{
	pc_lpt_state *lpt = get_safe_token_pc_lpt(device->owner());
