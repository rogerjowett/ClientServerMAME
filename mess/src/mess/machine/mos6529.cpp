...
***************************************************************************/
 ...
 mos6529_t *get_safe_token_mos6529(running_device *device)
{
	assert(device != NULL);
	return (mos6529_t *)downcast<legacy_device_base *>(device)->token();
}

 const mos6529_interface *get_interface_mos6529(running_device *device)
{
	assert(device != NULL);
...
{
	mos6529_t *mos6529 = get_safe_token_mos6529(device);
...
static DEVICE_START( mos6529 )
{
	...
	const mos6529_interface *intf = get_interface_mos6529(device);
