...
***************************************************************************/
 ...
 cbm_iec_t *get_safe_token_cbmiec(running_device *device)
{
	assert(device != NULL);
 ...
 const cbm_iec_daisy_chain *get_interface_cbmiec(running_device *device)
{
	assert(device != NULL);
	assert(device->type() == CBM_IEC);
...
{
	cbm_iec_t *cbm_iec = get_safe_token_cbmiec(device);
...
{
	cbm_iec_t *cbm_iec = get_safe_token_cbmiec(iec);
	cbm_iec_daisy_state *daisy = cbm_iec->daisy_state;
	int data = 1;
...
{
	cbm_iec_t *cbm_iec = get_safe_token_cbmiec(device);
	const cbm_iec_daisy_chain *daisy = get_interface_cbmiec(device);
	int i;
