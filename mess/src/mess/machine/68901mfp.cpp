...
***************************************************************************/
 ...
 mc68901_t *get_safe_token_mc68901(running_device *device)
{
	assert(device != NULL);
	return (mc68901_t *)downcast<legacy_device_base *>(device)->token();
}

 const mc68901_interface *get_interface_68901mfp(running_device *device)
{
	assert(device != NULL);
...
{
	mc68901_t *mc68901 = get_safe_token_mc68901(device);
...
{
	mc68901_t *mc68901 = (mc68901_t *)get_safe_token_mc68901(device);
...
static DEVICE_START( mc68901 )
{
	...
	const mc68901_interface *intf = get_interface_68901mfp(device);
