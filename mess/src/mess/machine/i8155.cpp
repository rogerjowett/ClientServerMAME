...
***************************************************************************/
 ...
 i8155_t *get_safe_token_i8155(running_device *device)
{
	assert(device != NULL);
	return (i8155_t *)downcast<legacy_device_base *>(device)->token();
}

 const i8155_interface *get_interface_i8155(running_device *device)
{
	assert(device != NULL);
...
static TIMER_CALLBACK( counter_tick )
{
	...
	i8155_t *i8155 = get_safe_token_i8155(device);
...
static DEVICE_START( i8155 )
{
	...
	const i8155_interface *intf = get_interface_i8155(device);
