...
void ncr5380_init( running_machine *machine, const struct NCR5380interface *ncr_interface )
{
	int i;
	...
	for (i = 0; i < ncr_interface->scsidevs->devs_present; i++)
	{
		SCSIAllocInstance( machine, ncr_interface->scsidevs->devices[i].scsiClass, &devices[ncr_interface->scsidevs->devices[i].scsiID], ncr_interface->scsidevs->devices[i].diskregion );
	}
...
void ncr5380_exit( const struct NCR5380interface *ncr_interface )
{
	int i;
	...
	for (i = 0; i < ncr_interface->scsidevs->devs_present; i++)
	{
		SCSIDeleteInstance( devices[ncr_interface->scsidevs->devices[i].scsiID] );
	}
}
