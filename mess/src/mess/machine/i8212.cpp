...
***************************************************************************/
 ...
 i8212_t *get_safe_token_i8212(running_device *device)
{
	assert(device != NULL);
	return (i8212_t *)downcast<legacy_device_base *>(device)->token();
}

 const i8212_interface *get_interface_i8212(running_device *device)
{
	assert(device != NULL);
...
{
	i8212_t *i8212 = get_safe_token_i8212(device);
...
{
	i8212_t *i8212 = get_safe_token_i8212(device);
	const i8212_interface *intf = get_interface_i8212(device);
...
DEFINE_LEGACY_DEVICE(I8212, i8212);
