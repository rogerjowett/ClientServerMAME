...
***************************************************************************/
 ...
 abc77_t *get_safe_token_abc77(running_device *device)
{
	assert(device != NULL);
	return (abc77_t *)downcast<legacy_device_base *>(device)->token();
}

 const abc77_interface *get_interface_abc77(running_device *device)
{
	assert(device != NULL);
...
static TIMER_DEVICE_CALLBACK( clock_tick )
{
	...
	abc77_t *abc77 = get_safe_token_abc77(device);
...
static DEVICE_START( abc77 )
{
	...
	const abc77_interface *intf = get_interface_abc77(device);
