...
***************************************************************************/
 ...
 mc6852_t *get_safe_token_mc6852(running_device *device)
{
	assert(device != NULL);
 ...
 const mc6852_interface *get_interface_mc6852(running_device *device)
{
	assert(device != NULL);
	assert(device->type() == MC6852);
...
{
	mc6852_t *mc6852 = get_safe_token_mc6852(device);
...
static DEVICE_START( mc6852 )
{
	...
	const mc6852_interface *intf = get_interface_mc6852(device);
