...
***************************************************************************/
 ...
 abcbus_t *get_safe_token_abcbus(running_device *device)
{
	assert(device != NULL);
 ...
 const abcbus_daisy_chain *get_interface_abcbus(running_device *device)
{
	assert(device != NULL);
	assert(device->type() == ABCBUS);
...
{
	abcbus_t *abcbus = get_safe_token_abcbus(device);
...
{
	abcbus_t *abcbus = get_safe_token_abcbus(device);
	const abcbus_daisy_chain *daisy = get_interface_abcbus(device);
