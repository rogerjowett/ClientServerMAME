...
***************************************************************************/
 ...
 rs232_t *get_safe_token_rs232(running_device *device)
{
	assert(device != NULL);
 ...
 const rs232_interface *get_interface_rs232(running_device *device)
{
	assert(device != NULL);
	assert(device->type() == RS232);
...
{
	rs232_t *rs232 = get_safe_token_rs232(device);
...
{
	rs232_t *rs232 = get_safe_token_rs232(bus);
