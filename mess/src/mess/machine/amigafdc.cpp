...
/*****************************************************************************
    INLINE FUNCTIONS
*****************************************************************************/
 amiga_fdc_t *get_safe_token_amigafdc(running_device *device)
{
	assert(device != NULL);
...
{
	amiga_fdc_t *fdc = get_safe_token_amigafdc(image.device().owner());
...
{
	amiga_fdc_t *fdc = get_safe_token_amigafdc(device);
	int id;
	for(id=0;id<2;id++)
	{
...
static TIMER_CALLBACK(fdc_sync_proc)
{
	...
	int				sector;
	int				time;
	amiga_fdc_t *fdc = get_safe_token_amigafdc((running_device*)ptr);
