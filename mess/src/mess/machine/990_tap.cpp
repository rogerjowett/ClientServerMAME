...
/*****************************************************************************
    INLINE FUNCTIONS
*****************************************************************************/
 tap_990_t *get_safe_token_990_tap(running_device *device)
{
	assert(device != NULL);
...
{
	tape_unit_t *t;
	tap_990_t *tpc = get_safe_token_990_tap(device->owner());
...
{
	tape_unit_t *t;
	tap_990_t *tpc = get_safe_token_990_tap(image.device().owner());
...
{
	int reply;
	tap_990_t *tpc = get_safe_token_990_tap(device);
