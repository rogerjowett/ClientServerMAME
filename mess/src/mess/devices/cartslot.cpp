...
DEVICE_GET_INFO( cartslot )
{
	switch(state)
	{
		...
		case DEVINFO_STR_IMAGE_INTERFACE:
			if ( device && downcast<const legacy_image_device_config_base *>(device)->inline_config() && get_config_dev(device)->cart_interface )
			{
				strcpy(info->s, get_config_dev(device)->cart_interface );
			}
			break;
	}
}
