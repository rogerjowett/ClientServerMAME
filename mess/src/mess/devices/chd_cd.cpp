...
struct _dev_cdrom_t
{
 ...
 dev_cdrom_t *get_safe_token_chd_cd(running_device *device) {
	assert( device != NULL );
	assert( ( device->type() == CDROM ) );
...
{
	dev_cdrom_t	*cdrom = get_safe_token_chd_cd(&image.device());
...
{
	dev_cdrom_t	*cdrom = get_safe_token_chd_cd( image );
...
{
	dev_cdrom_t	*cdrom = get_safe_token_chd_cd( device );
...
DEVICE_GET_INFO(cdrom)
{
	switch( state )
	{
		...
		case DEVINFO_STR_IMAGE_INTERFACE:
			if ( device && downcast<const legacy_image_device_config_base *>(device)->inline_config() && get_config_dev(device)->cdrom_interface )
			{
				strcpy(info->s, get_config_dev(device)->cdrom_interface );
			}
			break;
