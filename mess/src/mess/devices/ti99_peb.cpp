...
typedef struct _ti99_peb_state
{
 ...
 ti99_peb_state *get_safe_token_ti99_peb(running_device *device)
{
	assert(device != NULL);
...
{
	int port;
	ti99_peb_state *peb = get_safe_token_ti99_peb(box);
...
READ8_DEVICE_HANDLER( ti99_4x_peb_cru_r )
{
	...
	int reply;
	ti99_peb_state *peb = get_safe_token_ti99_peb(device);
