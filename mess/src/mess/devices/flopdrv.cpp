...
DECLARE_LEGACY_IMAGE_DEVICE(FLOPPY_SONY, sonydriv);
 ...
 floppy_drive *get_safe_token_flopdrv(running_device *device)
{
	assert( device != NULL );
...
{
	return get_safe_token_flopdrv(image)->floppy;
}

void *flopimg_get_custom_data(running_device *image)
{
	floppy_drive *flopimg = get_safe_token_flopdrv( image );
...
static void flopimg_get_id_callback(running_device *image, chrn_id *id, int id_index, int side)
{
	...
	flopimg = get_safe_token_flopdrv( image );
	if (!flopimg || !flopimg->floppy)
		return;
...
{
	floppy_drive *pDrive = get_safe_token_flopdrv( img );
...
{
	floppy_drive *drive = get_safe_token_flopdrv( img );
...
{
	floppy_drive *drv = get_safe_token_flopdrv( img );
	int prev_state;
	int new_state;
...
void floppy_drive_seek(running_device *img, signed int signed_tracks)
{
	...
	pDrive = get_safe_token_flopdrv( img );
...
void floppy_drive_read_track_data_info_buffer(running_device *img, int side, void *ptr, int *length )
{
	...
	if (image->exists())
	{
		flopimg = get_safe_token_flopdrv( img );
		if (!flopimg || !flopimg->floppy)
			return;
...
DEVICE_START( floppy )
{
	floppy_drive *floppy = get_safe_token_flopdrv( device );
...
static int internal_floppy_device_load(device_image_interface *image, int create_format, option_resolution *create_args)
{
	...
	flopimg = get_safe_token_flopdrv( &image->device() );
...
{
	floppy_drive *flopimg = get_safe_token_flopdrv( &image.device() );
...
{
	floppy_drive *drive = get_safe_token_flopdrv(device);
...
DEVICE_GET_INFO(floppy)
{
	switch( state )
	{
		...
		case DEVINFO_STR_IMAGE_INTERFACE:
			if ( device && device->static_config() && ((floppy_config *)device->static_config())->floppy_interface)
			{
				strcpy(info->s, ((floppy_config *)device->static_config())->floppy_interface );
			}
			break;
		default:
			{
