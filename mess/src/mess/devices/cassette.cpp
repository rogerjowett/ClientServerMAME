...
const cassette_config default_cassette_config =
{
 ...
 dev_cassette_t *get_safe_token_cassette(running_device *device)
{
	assert( device != NULL );
...
{
	dev_cassette_t	*cassette = get_safe_token_cassette( device );
...
DEVICE_GET_INFO(cassette)
{
	switch( state )
	{
		...
		case DEVINFO_STR_IMAGE_INTERFACE:
			if ( device && device->static_config() && ((cassette_config *)device->static_config())->cassette_interface)
			{
				strcpy(info->s, ((cassette_config *)device->static_config())->cassette_interface );
			}
			break;			
	}
}
