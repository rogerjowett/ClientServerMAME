...
struct _dmadac_state
{
 ...
 dmadac_state *get_safe_token_dmadac(running_device *device)
{
	assert(device != NULL);
...
{
	dmadac_state *info = get_safe_token_dmadac(device);
...
void dmadac_transfer(dmadac_sound_device **devlist, UINT8 num_channels, offs_t channel_spacing, offs_t frame_spacing, offs_t total_frames, INT16 *data)
{
	...
	{
		dmadac_state *info = get_safe_token_dmadac(devlist[i]);
	...
	for (i = 0; i < num_channels; i++)
	{
		dmadac_state *ch = get_safe_token_dmadac(devlist[i]);
		if (ch->enabled)
		{
