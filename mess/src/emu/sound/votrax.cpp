...
struct _votrax_state
{
 ...
 votrax_state *get_safe_token_votrax(running_device *device)
{
	assert(device != NULL);
...
{
	votrax_state *votrax = get_safe_token_votrax(device);
...
{
	votrax_state *info = get_safe_token_votrax(device);
