...
struct _nesapu_state
{
 ...
 nesapu_state *get_safe_token_nes_apu(running_device *device)
{
	assert(device != NULL);
...
READ8_DEVICE_HANDLER( nes_psg_r ) {return apu_read(get_safe_token_nes_apu(device),offset);}
WRITE8_DEVICE_HANDLER( nes_psg_w ) {apu_write(get_safe_token_nes_apu(device),offset,data);}
...
static DEVICE_START( nesapu )
{
	...
	nesapu_state *info = get_safe_token_nes_apu(device);
