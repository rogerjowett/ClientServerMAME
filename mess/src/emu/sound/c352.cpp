...
struct _c352_state
{
 ...
 c352_state *get_safe_token_c352(running_device *device)
{
	assert(device != NULL);
...
{
	c352_state *info = get_safe_token_c352(device);
...
{
	return(c352_read_reg16(get_safe_token_c352(device), offset*2));
}
...
WRITE16_DEVICE_HANDLER( c352_w )
{
	if (mem_mask == 0xffff)
	{
		c352_write_reg16(get_safe_token_c352(device), offset*2, data);
	}
	else
	{
