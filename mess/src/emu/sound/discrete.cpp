...
#include "disc_sys.cpp"		/* discrete core modules and support functions */
...
static DEVICE_START( discrete )
{
	...
	discrete_info *info = get_safe_token_discrete(device);
	char name[32];
...
static DEVICE_RESET( discrete )
{
	const discrete_info *info = get_safe_token_discrete(device);
	const linked_list_entry *entry;
