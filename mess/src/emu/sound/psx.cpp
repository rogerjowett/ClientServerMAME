...
struct psxinfo
{
 ...
 struct psxinfo *get_safe_token_psx_sound(running_device *device)
{
	assert(device != NULL);
...
{
	struct psxinfo *chip = get_safe_token_psx_sound(machine->device("spu"));
...
{
	struct psxinfo *chip = get_safe_token_psx_sound(device);
	int n_effect;
	int n_channel;
