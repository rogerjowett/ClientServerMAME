...
struct _ym2151_state
{
 ...
 ym2151_state *get_safe_token_2151intf(running_device *device)
{
	assert(device != NULL);
...
static DEVICE_START( ym2151 )
{
	...
	ym2151_state *info = get_safe_token_2151intf(device);
	int rate;
...
{
	ym2151_state *token = get_safe_token_2151intf(device);
