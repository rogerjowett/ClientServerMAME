...
typedef struct
{
 ...
 esrip_state *get_safe_token_esrip(running_device *device)
{
	assert(device != NULL);
...
{
	esrip_state *cpustate = get_safe_token_esrip(cpu);
...
{
	esrip_state *cpustate = get_safe_token_esrip(device);
...
CPU_GET_INFO( esrip )
{
	esrip_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_esrip(device) : NULL;
