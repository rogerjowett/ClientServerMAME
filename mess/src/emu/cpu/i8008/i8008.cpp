...
***************************************************************************/
 ...
 i8008_state *get_safe_token_i8008(running_device *device)
{
	assert(device != NULL);
...
{
	i8008_state *cpustate = get_safe_token_i8008(device);
...
CPU_GET_INFO( i8008 )
{
	i8008_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_i8008(device) : NULL;
	switch (state)
	{
