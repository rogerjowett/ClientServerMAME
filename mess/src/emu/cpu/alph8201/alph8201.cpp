...
#define LP2				lp2
 ...
 alpha8201_state *get_safe_token_alph8201(running_device *device)
{
	assert(device != NULL);
...
{
	alpha8201_state *cpustate = get_safe_token_alph8201(device);
...
{
	alpha8201_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_alph8201(device) : NULL;
	switch (state)
	{
