...
struct _t11_state
{
 ...
 t11_state *get_safe_token_t11(running_device *device)
{
	assert(device != NULL);
...
static CPU_INIT( t11 )
{
	...
	t11_state *cpustate = get_safe_token_t11(device);
...
CPU_GET_INFO( t11 )
{
	t11_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_t11(device) : NULL;
