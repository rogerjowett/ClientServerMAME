...
struct _superfx_state
{
 ...
 superfx_state *get_safe_token_superfx(running_device *device)
{
	assert(device != NULL);
...
{
	superfx_state *cpustate = get_safe_token_superfx(cpu);
...
{
	superfx_state *cpustate = get_safe_token_superfx(device);
	int i;
...
{
	superfx_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_superfx(device) : NULL;
