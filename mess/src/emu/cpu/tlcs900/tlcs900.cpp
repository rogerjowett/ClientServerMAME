...
#define WRMEML(addr,data)		{ UINT32 dl = data; WRMEMW(addr,dl); WRMEMW(addr+2,(dl >> 16)); }
 ...
 tlcs900_state *get_safe_token_tlcs900( running_device *device )
{
	assert( device != NULL );
...
{
	tlcs900_state *cpustate = get_safe_token_tlcs900(device);
...
{
	tlcs900_state *cpustate = get_safe_token_tlcs900( space->cpu );
...
{
	tlcs900_state *cpustate = ( device != NULL && device->token() != NULL ) ? get_safe_token_tlcs900(device) : NULL;
