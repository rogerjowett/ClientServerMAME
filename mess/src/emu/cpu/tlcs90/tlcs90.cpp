...
typedef struct
{
 ...
 t90_Regs *get_safe_token_tlcs90(running_device *device)
{
	assert(device != NULL);
...
CPU_DISASSEMBLE( t90 )
{
	t90_Regs *cpustate = get_safe_token_tlcs90(device);
	int len;
...
{
	t90_Regs *cpustate = get_safe_token_tlcs90(space->cpu);
...
{
	t90_Regs *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_tlcs90(device) : NULL;
