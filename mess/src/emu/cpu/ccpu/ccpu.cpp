...
struct _ccpu_state
{
 ...
 ccpu_state *get_safe_token_ccpu(running_device *device)
{
	assert(device != NULL);
...
static UINT8 read_jmi(running_device *device)
{
	...
	ccpu_state *cpustate = get_safe_token_ccpu(device);
...
CPU_GET_INFO( ccpu )
{
	ccpu_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_ccpu(device) : NULL;
