...
INLINE void mn102_change_pc(mn102_info *mn102, UINT32 pc)
{
 ...
 mn102_info *get_safe_token_mn10200(running_device *device)
{
	assert(device != NULL);
...
{
	mn102_info *cpustate = get_safe_token_mn10200(device);
	int tmr;
...
{
	mn102_info *mn102 = get_safe_token_mn10200(device);
...
{
	mn102_info *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_mn10200(device) : NULL;
