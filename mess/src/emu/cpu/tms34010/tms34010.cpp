...
struct _tms34010_state
{
 ...
 tms34010_state *get_safe_token_tms34010(running_device *device)
{
	assert(device != NULL);
...
static CPU_INIT( tms34010 )
{
	...
	tms34010_state *tms = get_safe_token_tms34010(device);
...
{
	tms34010_state *tms = get_safe_token_tms34010(cpu);
...
VIDEO_UPDATE( tms340x0 )
{
	...
	for (cpu = screen->machine->m_devicelist.first(); cpu != NULL; cpu = cpu->next())
	{
		...
		{
			tms = get_safe_token_tms34010(cpu);
...
{
	tms34010_state *tms = get_safe_token_tms34010(space->cpu);
	int oldreg, newreg;
...
{
	tms34010_state *tms = (device != NULL && device->token() != NULL) ? get_safe_token_tms34010(device) : NULL;
