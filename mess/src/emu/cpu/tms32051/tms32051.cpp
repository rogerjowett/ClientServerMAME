...
struct _tms32051_state
{
 ...
 tms32051_state *get_safe_token_tms32051(running_device *device)
{
	assert(device != NULL);
...
static CPU_INIT( tms )
{
	tms32051_state *cpustate = get_safe_token_tms32051(device);
...
{
	tms32051_state *cpustate = get_safe_token_tms32051(space->cpu);
...
{
	tms32051_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_tms32051(device) : NULL;
