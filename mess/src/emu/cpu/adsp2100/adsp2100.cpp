...
***************************************************************************/
 ...
 adsp2100_state *get_safe_token_adsp2100(running_device *device)
{
	assert(device != NULL);
...
static adsp2100_state *adsp21xx_init(legacy_cpu_device *device, device_irq_callback irqcallback, int chiptype)
{
	...
	adsp2100_state *adsp = get_safe_token_adsp2100(device);
...
{
	adsp2100_state *adsp = (device != NULL && device->token() != NULL) ? get_safe_token_adsp2100(device) : NULL;
	switch (state)
	{
