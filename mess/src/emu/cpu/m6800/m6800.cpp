...
//  int     subtype;        /* CPU subtype */
 ...
 m6800_state *get_safe_token_m6800(running_device *device)
{
	assert(device != NULL);
...
{
	m6800_state *cpustate = get_safe_token_m6800(device);
...
static READ8_HANDLER( m6803_internal_registers_r )
{
	m6800_state *cpustate = get_safe_token_m6800(space->cpu);
...
CPU_GET_INFO( m6800 )
{
	m6800_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_m6800(device) : NULL;
	switch (state)
	{
