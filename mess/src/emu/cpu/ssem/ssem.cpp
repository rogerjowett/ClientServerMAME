...
struct _ssem_state
{
 ...
 ssem_state *get_safe_token_ssem(running_device *device)
{
    assert(device != NULL);
...
{
    ssem_state *cpustate = get_safe_token_ssem(device);
    cpustate->pc = 1;
    cpustate->a = 0;
    cpustate->halt = 0;
...
CPU_GET_INFO( ssem )
{
    ssem_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_ssem(device) : NULL;
