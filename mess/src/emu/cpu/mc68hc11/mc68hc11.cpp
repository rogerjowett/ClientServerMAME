...
struct _hc11_state
{
 ...
 hc11_state *get_safe_token_mc68hc11(running_device *device)
{
	assert(device != NULL);
...
{
	hc11_state *cpustate = get_safe_token_mc68hc11(device);
	int i;
...
{
	hc11_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_mc68hc11(device) : NULL;
