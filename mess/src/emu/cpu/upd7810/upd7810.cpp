...
/* internal helper variables */
 ...
 upd7810_state *get_safe_token_upd7810(running_device *device)
{
	assert(device != NULL);
...
{
	upd7810_state *cpustate = get_safe_token_upd7810(device);
...
{
	upd7810_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_upd7810(device) : NULL;
	switch (state)
	{
