...
union _lr35902_state {
 ...
 lr35902_state *get_safe_token_lr35902(running_device *device)
{
	assert(device != NULL);
...
{
	lr35902_state *cpustate = get_safe_token_lr35902(device);
...
{
	lr35902_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_lr35902(device) : NULL;
