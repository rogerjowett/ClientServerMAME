...
inline sh4_state *get_safe_token_sh4(running_device *device)
{
	assert(device != NULL);
...
/* SH_FLOAT FPUL,FRn PR=0 1111nnnn00101101 */
/* SH_FLOAT FPUL,DRn PR=1 1111nnn000101101 */
INLINE void SH_FLOAT(sh4_state *sh4, UINT32 n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
...
{
	sh4_state *sh4 = get_safe_token_sh4(device);
	emu_timer *tsaved[4];
	emu_timer *tsave[5];
	UINT32 *m;
	int save_is_slave;
...
{
	sh4_state *sh4 = get_safe_token_sh4(space->cpu);
...
CPU_GET_INFO( sh4 )
{
	sh4_state *sh4 = (device != NULL && device->token() != NULL) ? get_safe_token_sh4(device) : NULL;
