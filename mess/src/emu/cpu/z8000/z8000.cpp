...
#include "z8000cpu.h"
 ...
 z8000_state *get_safe_token_z8000(running_device *device)
{
	assert(device != NULL);
...
{
	z8000_state *cpustate = get_safe_token_z8000(device);
...
CPU_GET_INFO( z8002 )
{
	z8000_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_z8000(device) : NULL;
