...
#include "h6280ops.h"
 ...
 h6280_Regs *get_safe_token_h6280(running_device *device)
{
	assert(device != NULL);
...
{
	h6280_Regs* cpustate = get_safe_token_h6280(device);
...
{
	int status;
	h6280_Regs *cpustate = get_safe_token_h6280(space->cpu);
...
CPU_GET_INFO( h6280 )
{
	h6280_Regs* cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_h6280(device) : NULL;
