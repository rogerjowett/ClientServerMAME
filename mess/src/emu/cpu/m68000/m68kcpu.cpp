...
#define MASK_040_OR_LATER			(CPU_TYPE_040 | CPU_TYPE_EC040)
 ...
 m68ki_cpu_core *get_safe_token_m68kcpu(running_device *device)
{
	assert(device != NULL);
...
{
	m68ki_cpu_core *m68k = get_safe_token_m68kcpu(device);
...
{
	m68ki_cpu_core *m68k = (device != NULL && device->token() != NULL) ? get_safe_token_m68kcpu(device) : NULL;
...
{
	m68ki_cpu_core *m68k = get_safe_token_m68kcpu(space->cpu);
