...
***************************************************************************/
 ...
 jaguar_state *get_safe_token_jaguar(running_device *device)
{
	assert(device != NULL);
...
static STATE_POSTLOAD( jaguar_postload )
{
	...
	jaguar_state *jaguar = get_safe_token_jaguar(device);
...
{
	jaguar_state *jaguar = (device != NULL && device->token() != NULL) ? get_safe_token_jaguar(device) : NULL;
	switch (state)
	{
