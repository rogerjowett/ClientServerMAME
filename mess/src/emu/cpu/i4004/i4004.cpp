...
***************************************************************************/
 ...
 i4004_state *get_safe_token_i4004(running_device *device)
{
	assert(device != NULL);
...
{
	i4004_state *cpustate = get_safe_token_i4004(device);
	cpustate->TEST = val;
}
...
CPU_GET_INFO( i4004 )
{
	i4004_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_i4004(device) : NULL;
	switch (state)
	{
