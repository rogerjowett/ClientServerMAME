...
***************************************************************************/
 ...
 sh2_state *get_safe_token_sh2drc(running_device *device)
{
	assert(device != NULL);
...
static CPU_INIT( sh2 )
{
	...
	sh2_state *sh2 = get_safe_token_sh2drc(device);
	drccache *cache;
	drcbe_info beinfo;
	UINT32 flags = 0;
	int regnum;
...
CPU_GET_INFO( sh2 )
{
	sh2_state *sh2 = (device != NULL && device->token() != NULL) ? get_safe_token_sh2drc(device) : NULL;
	switch (state)
	{
