...
#define LOG(x)	do { if (VERBOSE) logerror x; } while (0)
 ...
 sh2_state *get_safe_token_sh2(running_device *device)
{
	assert(device != NULL);
...
{
	sh2_state *sh2 = get_safe_token_sh2(device);
...
CPU_GET_INFO( sh2 )
{
	sh2_state *sh2 = (device != NULL && device->token() != NULL) ? get_safe_token_sh2(device) : NULL;
	switch (state)
	{
