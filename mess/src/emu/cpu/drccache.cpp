...
{
	/* core parameters */
	drccodeptr			dr_near;				/* pointer to the near part of the cache */
...
drccache *drccache_alloc(size_t bytes)
{
	...
	cache.dr_near = (drccodeptr)osd_alloc_executable(bytes);
	cache.neartop = cache.dr_near;
	cache.base = cache.dr_near + NEAR_CACHE_SIZE;
	cache.top = cache.base;
	cache.end = cache.dr_near + bytes;
	cache.size = bytes;
...
void drccache_free(drccache *cache)
{
	...
	osd_free_executable(cache->dr_near, cache->size);
}
...
{
	return ((const drccodeptr)ptr >= cache->dr_near && (const drccodeptr)ptr < cache->dr_near + cache->size);
}
...
{
	return ((const drccodeptr)ptr >= cache->dr_near && (const drccodeptr)ptr < cache->neartop);
}
...
void drccache_memory_free(drccache *cache, void *memory, size_t bytes)
{
	...
	assert(((drccodeptr)memory >= cache->dr_near && (drccodeptr)memory < cache->base) || ((drccodeptr)memory >= cache->end && (drccodeptr)memory < cache->dr_near + cache->size));
