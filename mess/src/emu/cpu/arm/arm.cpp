...
/***************************************************************************/
 ...
 ARM_REGS *get_safe_token_arm(running_device *device)
{
	assert(device != NULL);
...
{
	ARM_REGS *cpustate = get_safe_token_arm(device);
...
CPU_GET_INFO( arm )
{
	ARM_REGS *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_arm(device) : NULL;
