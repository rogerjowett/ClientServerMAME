...
inline m68_state_t *get_safe_token_m6809(running_device *device)
{
	assert(device != NULL);
...
static CPU_INIT( m6809 )
{
	...
	m68_state_t *m68_state = get_safe_token_m6809(device);
...
CPU_GET_INFO( m6809 )
{
	m68_state_t *m68_state = (device != NULL && device->token() != NULL) ? get_safe_token_m6809(device) : NULL;
