...
ADDRESS_MAP_END
 ...
 hyperstone_state *get_safe_token_e132xs(running_device *device)
{
	assert(device != NULL);
...
static TIMER_CALLBACK( e132xs_timer_callback )
{
	...
	hyperstone_state *cpustate = get_safe_token_e132xs(device);
	int update = param;
...
{
	hyperstone_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_e132xs(device) : NULL;
	switch (state)
	{
