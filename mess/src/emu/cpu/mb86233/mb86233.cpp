...
struct _mb86233_state
{
 ...
 mb86233_state *get_safe_token_mb86233(running_device *device)
{
	assert(device != NULL);
...
{
	mb86233_state *cpustate = get_safe_token_mb86233(device);
...
{
	mb86233_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_mb86233(device) : NULL;
