...
struct _i960_state_t {
 ...
 i960_state_t *get_safe_token_i960(running_device *device)
{
	assert(device != NULL);
...
{
	i960_state_t *i960 = get_safe_token_i960(device);
	UINT32 opcode;
...
CPU_GET_INFO( i960 )
{
	i960_state_t *i960 = (device != NULL && device->token() != NULL) ? get_safe_token_i960(device) : NULL;
