...
struct _psxcpu_state
{
 ...
 psxcpu_state *get_safe_token_psx(running_device *device)
{
	assert(device != NULL);
...
{
	psxcpu_state *psxcpu = get_safe_token_psx(space->cpu);
...
{
	psxcpu_state *psxcpu = get_safe_token_psx(device);
...
CPU_GET_INFO( psxcpu )
{
	psxcpu_state *psxcpu = (device != NULL && device->token() != NULL) ? get_safe_token_psx(device) : NULL;
	switch (state)
	{
