...
struct _r3000_state
{
 ...
 r3000_state *get_safe_token_r3000(running_device *device)
{
	assert(device != NULL);
...
static CPU_INIT( r3000 )
{
	...
	r3000_state *r3000 = get_safe_token_r3000(device);
...
{
	r3000_reset(get_safe_token_r3000(device), 1);
}

static CPU_RESET( r3000le )
{
	r3000_reset(get_safe_token_r3000(device), 0);
}
...
static UINT8 readcache_be(const address_space *space, offs_t offset)
{
	r3000_state *r3000 = get_safe_token_r3000(space->cpu);
	offset &= 0x1fffffff;
...
{
	r3000_state *r3000 = (device != NULL && device->token() != NULL) ? get_safe_token_r3000(device) : NULL;
	switch (state)
	{
