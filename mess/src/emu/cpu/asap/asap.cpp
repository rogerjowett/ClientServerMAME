...
***************************************************************************/
 ...
 asap_state *get_safe_token_asap(running_device *device)
{
	assert(device != NULL);
...
{
	asap_state *asap = get_safe_token_asap(device);
	int i;
...
CPU_GET_INFO( asap )
{
	asap_state *asap = (device != NULL && device->token() != NULL) ? get_safe_token_asap(device) : NULL;
