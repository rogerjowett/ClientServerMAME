...
#define ARM7_ICOUNT         cpustate->iCount
 ...
 arm_state *get_safe_token_arm7(running_device *device)
{
	assert(device != NULL);
...
{
	arm_state *cpustate = get_safe_token_arm7(device);
...
CPU_GET_INFO( arm7 )
{
    arm_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_arm7(device) : NULL;
