...
spc700i_cpu *get_safe_token_spc700(running_device *device)
{
	assert(device != NULL);
...
{
	spc700i_cpu *cpustate = get_safe_token_spc700(device);
...
CPU_GET_INFO( spc700 )
{
	spc700i_cpu *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_spc700(device) : NULL;
	uint p = 0;
