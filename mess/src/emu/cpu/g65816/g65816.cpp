...
g65816i_cpu_struct *get_safe_token_g65816(running_device *device)
{
	assert(device != NULL);
...
{
	g65816i_cpu_struct *cpustate = get_safe_token_g65816(device);
...
CPU_GET_INFO( g65816 )
{
	g65816i_cpu_struct *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_g65816(device) : NULL;
