...
inline m68_state_t *get_safe_token_hd6309(running_device *device)
{
	assert(device != NULL);
...
static STATE_POSTLOAD( hd6309_postload )
{
	...
	m68_state_t *m68_state = get_safe_token_hd6309(device);
...
CPU_GET_INFO( hd6309 )
{
	m68_state_t *m68_state = (device != NULL && device->token() != NULL) ? get_safe_token_hd6309(device) : NULL;
