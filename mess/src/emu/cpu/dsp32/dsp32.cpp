...
***************************************************************************/
 ...
 dsp32_state *get_safe_token_dsp32(running_device *device)
{
	assert(device != NULL);
...
{
	dsp32_state *cpustate = get_safe_token_dsp32(device);
...
CPU_GET_INFO( dsp32c )
{
	dsp32_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_dsp32(device) : NULL;
	switch (state)
	{
