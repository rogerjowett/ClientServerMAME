...
struct _lh5810_state
{
 ...
 lh5801_state *get_safe_token_lh5801(running_device *device)
{
	assert(device != NULL);
...
{
	lh5801_state *cpustate = get_safe_token_lh5801(device);
...
CPU_GET_INFO( lh5801 )
{
	lh5801_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_lh5801(device) : NULL;
