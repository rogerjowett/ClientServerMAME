...
#define MP 8 // Module Pulled
 ...
 saturn_state *get_safe_token_saturn(running_device *device)
{
	assert(device != NULL);
...
{
	saturn_state *cpustate = get_safe_token_saturn(device);
...
CPU_GET_INFO( saturn )
{
	saturn_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_saturn(device) : NULL;
