...
struct _v810_state
{
 ...
 v810_state *get_safe_token_v810(running_device *device)
{
	assert(device != NULL);
...
{
	v810_state *cpustate = get_safe_token_v810(device);
...
CPU_GET_INFO( v810 )
{
	v810_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_v810(device) : NULL;
