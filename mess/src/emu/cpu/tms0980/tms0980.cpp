...
static const UINT32 tms1100_default_decode[256] = {
 ...
 tms0980_state *get_safe_token_tms0980(running_device *device)
{
	assert(device != NULL);
...
{
	tms0980_state *cpustate = get_safe_token_tms0980( device );
...
{
	tms0980_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_tms0980(device) : NULL;
