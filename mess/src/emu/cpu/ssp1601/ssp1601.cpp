...
struct _ssp1601_state_t
{
 ...
 ssp1601_state_t *get_safe_token_ssp1601(running_device *device)
{
	assert(device != NULL);
...
{
	ssp1601_state_t *ssp1601_state = get_safe_token_ssp1601(device);
...
{
	ssp1601_state_t *ssp1601_state = (device != NULL && device->token() != NULL) ? get_safe_token_ssp1601(device) : NULL;
