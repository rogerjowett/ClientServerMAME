...
***************************************************************************/
 ...
 scmp_state *get_safe_token_scmp(running_device *device)
{
	assert(device != NULL);
...
{
	scmp_state *cpustate = get_safe_token_scmp(device);
...
CPU_GET_INFO( scmp )
{
	scmp_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_scmp(device) : NULL;
	switch (state)
	{
