...
#define ROPCODE(pc)		((UINT64)(cpustate->internal_ram[((pc-0x20000) * 3) + 0]) << 32) | \
 ...
 SHARC_REGS *get_safe_token_sharc(running_device *device)
{
	assert(device != NULL);
...
#include "sharcmem.cpp"
#include "sharcdma.cpp"
#include "sharcops.cpp"
#include "sharcops.h"
...
{
	SHARC_REGS *cpustate = get_safe_token_sharc(device);
	if (address == 0x1c)
	{
		if (data != 0)
		{
...
static CPU_SET_INFO( adsp21062 )
{
	...
	{
		sharc_set_irq_line(get_safe_token_sharc(device), state-CPUINFO_INT_INPUT_STATE, info->i);
		return;
	}
...
{
	SHARC_REGS *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_sharc(device) : NULL;
