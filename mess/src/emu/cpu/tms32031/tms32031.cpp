...
struct _tms32031_state
{
 ...
 tms32031_state *get_safe_token_tms32031(running_device *device)
{
	assert(device != NULL);
...
static CPU_INIT( tms32031 )
{
	...
	tms32031_state *tms = get_safe_token_tms32031(device);
	int i;
...
{
	tms32031_state *tms = (device != NULL && device->token() != NULL) ? get_safe_token_tms32031(device) : NULL;
	float ftemp;
