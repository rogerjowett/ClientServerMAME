...
#include "unsp.h"
 ...
 unsp_state *get_safe_token_unsp(running_device *device)
{
    assert(device != NULL);
...
{
    unsp_state *unsp = get_safe_token_unsp(device);
...
CPU_GET_INFO( unsp )
{
    unsp_state *unsp = (device != NULL && device->token() != NULL) ? get_safe_token_unsp(device) : NULL;
