...
static _OP *OpTable=NULL;
 ...
 se3208_state_t *get_safe_token_se3208(running_device *device)
{
	assert(device != NULL);
...
{
	se3208_state_t *se3208_state = get_safe_token_se3208(device);
...
CPU_GET_INFO( se3208 )
{
	se3208_state_t *se3208_state = (device != NULL && device->token() != NULL) ? get_safe_token_se3208(device) : NULL;
