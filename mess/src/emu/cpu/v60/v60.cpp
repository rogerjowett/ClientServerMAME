...
struct _v60_state
{
 ...
 v60_state *get_safe_token_v60(running_device *device)
{
	assert(device != NULL);
...
{
	v60_state *cpustate = get_safe_token_v60(device);
...
CPU_GET_INFO( v60 )
{
	v60_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_v60(device) : NULL;
