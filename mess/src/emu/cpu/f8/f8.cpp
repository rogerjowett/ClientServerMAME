...
struct _f8_Regs
{
 ...
 f8_Regs *get_safe_token_f8(running_device *device)
{
	assert(device != NULL);
...
static CPU_RESET( f8 )
{
	f8_Regs *cpustate = get_safe_token_f8(device);
	UINT8 data;
	int i;
...
CPU_GET_INFO( f8 )
{
	f8_Regs *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_f8(device) : NULL;
