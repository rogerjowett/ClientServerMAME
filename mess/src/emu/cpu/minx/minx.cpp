...
#define GET_MINX_PC		( ( minx->PC & 0x8000 ) ? ( minx->V << 15 ) | (minx->PC & 0x7FFF ) : minx->PC )
 ...
 minx_state *get_safe_token_minx(running_device *device)
{
	assert(device != NULL);
...
{
	minx_state *minx = get_safe_token_minx(device);
...
CPU_GET_INFO( minx )
{
	minx_state *minx = (device != NULL && device->token() != NULL) ? get_safe_token_minx(device) : NULL;
	switch( state )
	{
