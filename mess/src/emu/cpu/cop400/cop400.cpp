...
***************************************************************************/
 ...
 cop400_state *get_safe_token_cop400(running_device *device)
{
	assert(device != NULL);
...
{
	cop400_state *cpustate = get_safe_token_cop400(device);
...
{
	cop400_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_cop400(device) : NULL;
