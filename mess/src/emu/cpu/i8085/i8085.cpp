...
***************************************************************************/
 ...
 i8085_state *get_safe_token_i8085(running_device *device)
{
	assert(device != NULL);
...
{
	i8085_state *cpustate = get_safe_token_i8085(device);
...
CPU_GET_INFO( i8085 )
{
	i8085_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_i8085(device) : NULL;
	switch (state)
	{
