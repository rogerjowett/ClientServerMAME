...
struct _m6502_Regs
{
 ...
 m6502_Regs *get_safe_token_m6502(running_device *device)
{
	assert(device != NULL);
...
{
	m6502_Regs *cpustate = get_safe_token_m6502(device);
...
{
	m6502_Regs *cpustate = get_safe_token_m6502(space->cpu);
	UINT8 result = 0x00;
...
CPU_GET_INFO( m6502 )
{
	m6502_Regs *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_m6502(device) : NULL;
