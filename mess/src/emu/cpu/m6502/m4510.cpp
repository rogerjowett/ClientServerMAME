...
struct _m4510_Regs {
 ...
 m4510_Regs *get_safe_token_m4510(running_device *device)
{
	assert(device != NULL);
...
{
	m4510_Regs *cpustate = get_safe_token_m4510(space->cpu);
...
{
	m4510_Regs *cpustate = get_safe_token_m4510(device);
...
CPU_GET_INFO( m4510 )
{
	m4510_Regs *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_m4510(device) : NULL;
