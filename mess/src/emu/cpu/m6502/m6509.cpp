...
struct _m6509_Regs {
 ...
 m6509_Regs *get_safe_token_m6509(running_device *device)
{
	assert(device != NULL);
...
{
	m6509_Regs *cpustate = get_safe_token_m6509(space->cpu);
...
{
	m6509_Regs *cpustate = get_safe_token_m6509(device);
...
CPU_GET_INFO( m6509 )
{
	m6509_Regs *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_m6509(device) : NULL;
