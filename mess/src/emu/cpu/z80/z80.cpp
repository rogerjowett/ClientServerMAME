...
struct _z80_state
{
 ...
 z80_state *get_safe_token_z80(running_device *device)
{
	assert(device != NULL);
...
static CPU_INIT( z80 )
{
	z80_state *z80 = get_safe_token_z80(device);
	int i, p;
...
{
	z80_state *cpustate = get_safe_token_z80(device);
...
CPU_GET_INFO( z80 )
{
	z80_state *z80 = (device != NULL && device->token() != NULL) ? get_safe_token_z80(device) : NULL;
	switch (state)
	{
