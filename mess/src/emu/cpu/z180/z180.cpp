...
struct _z180_state
{
 ...
 z180_state *get_safe_token_z180(running_device *device)
{
	assert(device != NULL);
...
{
	z180_state *cpustate = get_safe_token_z180(device);
...
CPU_GET_INFO( z180 )
{
	z180_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_z180(device) : NULL;
	switch (state)
	{
