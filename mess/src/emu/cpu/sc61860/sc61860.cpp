...
struct _sc61860_state
{
 ...
 sc61860_state *get_safe_token_sc61860(running_device *device)
{
	assert(device != NULL);
...
{
	sc61860_state *cpustate = get_safe_token_sc61860(device);
	return cpustate->ram;
}
...
{
	sc61860_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_sc61860(device) : NULL;
	switch (state)
	{
