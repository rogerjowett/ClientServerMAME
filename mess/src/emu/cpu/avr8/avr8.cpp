...
#define SPREG           ((READ_IO_8(cpustate, AVR8_IO_SPH) << 8) | READ_IO_8(cpustate, AVR8_IO_SPL))
 ...
 avr8_state *get_safe_token_avr8(running_device *device)
{
    assert(device != NULL);
...
{
    avr8_state *cpustate = get_safe_token_avr8(device);
...
CPU_GET_INFO( avr8 )
{
    avr8_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_avr8(device) : NULL;
