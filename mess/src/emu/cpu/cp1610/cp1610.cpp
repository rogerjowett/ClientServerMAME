...
struct _cp1610_state
{
 ...
 cp1610_state *get_safe_token_cp1610(running_device *device)
{
	assert(device != NULL);
...
{
	cp1610_state *cpustate = get_safe_token_cp1610(device);
	UINT16 opcode;
...
CPU_GET_INFO( cp1610 )
{
	cp1610_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_cp1610(device) : NULL;
	switch (state)
	{
	/* --- the following bits of info are returned as 64-bit signed integers --- */
