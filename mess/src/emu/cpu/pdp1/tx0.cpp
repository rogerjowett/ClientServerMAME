...
struct _tx0_state
{
 ...
 tx0_state *get_safe_token_tx0(running_device *device)
{
	assert(device != NULL);
...
{
	tx0_state *cpustate = get_safe_token_tx0(device);
...
{
	tx0_state *cpustate = ( device != NULL && device->token() != NULL ) ? get_safe_token_tx0(device) : NULL;
