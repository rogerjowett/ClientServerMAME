...
struct _pdp1_state
{
 ...
 pdp1_state *get_safe_token_pdp1(running_device *device)
{
	assert(device != NULL);
...
static CPU_INIT( pdp1 )
{
	...
	pdp1_state *cpustate = get_safe_token_pdp1(device);
	int i;
...
CPU_GET_INFO( pdp1 )
{
	pdp1_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_pdp1(device) : NULL;
