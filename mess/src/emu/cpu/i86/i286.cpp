...
struct _i80286_state
{
 ...
 i80286_state *get_safe_token_i286(running_device *device)
{
	assert(device != NULL);
...
{
	i80286_state *cpustate = get_safe_token_i286(device);
	static int urinit=1;
...
CPU_GET_INFO( i80286 )
{
	i80286_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_i286(device) : NULL;
