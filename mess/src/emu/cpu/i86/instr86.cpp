...
static void PREFIX86(_interrupt)(i8086_state *cpustate, unsigned int_num)
{
	...
	HOST_WORD ip = cpustate->pc - cpustate->base[CS];
	...
	if (PM) {
		...
		PUSH(ip);
		cpustate->sregs[CS] = (HOST_WORD)dest_seg;
...
static void PREFIX86(_rotate_shift_Byte)(i8086_state *cpustate, unsigned ModRM, unsigned count)
{
	...
	else
	{
		...
		case 0x00:	/* ROL eb,count */
			...
			PutbackRMByte(ModRM,(HOST_BYTE)dst);
			break;
...
static void PREFIX86(_83pre)(i8086_state *cpustate)    /* Opcode 0x83 */
{
	unsigned ModRM = FETCH;
    unsigned dst = GetRMWord(ModRM);
    unsigned src = (HOST_WORD)((INT16)((INT8)FETCH));
...
#else
	cpustate->sregs[CS] = (HOST_WORD)tmp2;
	cpustate->base[CS] = SegBase(CS);
	cpustate->pc = (cpustate->base[CS] + (HOST_WORD)tmp) & AMASK;
#endif
...
{
	HOST_BYTE tmp = GetMemB(DS,cpustate->regs.w[SI]);
	PutMemB(ES,cpustate->regs.w[DI], tmp);
	cpustate->regs.w[DI] += cpustate->DirVal;
	cpustate->regs.w[SI] += cpustate->DirVal;
	ICOUNT -= timing.movs8;
...
{
	HOST_WORD tmp = GetMemW(DS,cpustate->regs.w[SI]);
	PutMemW(ES,cpustate->regs.w[DI], tmp);
	cpustate->regs.w[DI] += 2 * cpustate->DirVal;
	cpustate->regs.w[SI] += 2 * cpustate->DirVal;
...
#else
	cpustate->sregs[CS] = (HOST_WORD)tmp1;
	cpustate->base[CS] = SegBase(CS);
...
static void PREFIX86(_f6pre)(i8086_state *cpustate)
{
    ...
    case 0x20:  /* MUL AL, Eb */
		ICOUNT -= (ModRM >= 0xc0) ? timing.mul_r8 : timing.mul_m8;
		{
			...
			cpustate->regs.w[AX]=(HOST_WORD)result;
...
static void PREFIX86(_fepre)(i8086_state *cpustate)    /* Opcode 0xfe */
{
    ...
    PutbackRMByte(ModRM,(HOST_BYTE)tmp1);
}
...
static void PREFIX86(_ffpre)(i8086_state *cpustate)    /* Opcode 0xff */
{
    ...
    case 0x00:  /* INC ew */
		...
		PutbackRMWord(ModRM,(HOST_WORD)tmp1);
		break;
