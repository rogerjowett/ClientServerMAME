...
struct _i8086_state
{
 ...
 i8086_state *get_safe_token_i86(running_device *device)
{
	assert(device != NULL);
...
{
	i8086_state *cpustate = get_safe_token_i86(device);
...
CPU_GET_INFO( i8086 )
{
	i8086_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_i86(device) : NULL;
