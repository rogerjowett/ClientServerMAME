...
struct _sm8500_state
{
 ...
 sm8500_state *get_safe_token_sm8500(running_device *device)
{
	assert(device != NULL);
...
{
	sm8500_state *cpustate = get_safe_token_sm8500(device);
...
CPU_GET_INFO( sm8500 )
{
	sm8500_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_sm8500(device) : NULL;
