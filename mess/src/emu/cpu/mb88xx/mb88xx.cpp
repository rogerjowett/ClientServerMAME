...
struct _mb88_state
{
 ...
 mb88_state *get_safe_token_mb88xx(running_device *device)
{
	assert(device != NULL);
...
{
	mb88_state *cpustate = get_safe_token_mb88xx(device);
...
static TIMER_CALLBACK( serial_timer )
{
	mb88_state *cpustate = get_safe_token_mb88xx((cpu_device *)ptr);
...
CPU_GET_INFO( mb88 )
{
	mb88_state *cpustate = (device != NULL && device->token() != NULL) ? get_safe_token_mb88xx(device) : NULL;
