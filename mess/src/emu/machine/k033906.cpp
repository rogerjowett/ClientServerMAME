...
INLINE k033906_state *k033906_get_safe_token_k033906( running_device *device )
{
	assert(device != NULL);
...
INLINE const k033906_interface *k033906_get_interface_k033906( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == K033906));
...
{
	k033906_state *k033906 = k033906_get_safe_token_k033906(device);
...
{
	k033906_state *k033906 = k033906_get_safe_token_k033906(device);
	const k033906_interface *intf = k033906_get_interface_k033906(device);
