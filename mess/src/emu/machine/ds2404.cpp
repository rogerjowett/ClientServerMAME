...
struct _ds2404_state {
 ...
 ds2404_state *get_safe_token_ds2404(running_device *device)
{
	assert(device != NULL);
...
{
	ds2404_state *state = get_safe_token_ds2404(device);
...
{
	ds2404_state *state = get_safe_token_ds2404((running_device *)ptr);
	int i;
	for( i = 0; i < 5; i++ )
	{
		state->rtc[ i ]++;
