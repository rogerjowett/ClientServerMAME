...
***************************************************************************/
 ...
 cdp1852_t *get_safe_token_cdp1852(running_device *device)
{
	assert(device != NULL);
	return (cdp1852_t *)downcast<legacy_device_base *>(device)->token();
}

 const cdp1852_interface *get_interface_cdp1852(running_device *device)
{
	assert(device != NULL);
...
static TIMER_CALLBACK( cdp1852_scan_tick )
{
	...
	cdp1852_t *cdp1852 = get_safe_token_cdp1852(device);
...
{
	cdp1852_t *cdp1852 = get_safe_token_cdp1852(device);
	const cdp1852_interface *intf = get_interface_cdp1852(device);
