...
void wd33c93_init( running_machine *machine, const struct WD33C93interface *wd_interface )
{
	int i;
	...
	for (i = 0; i < wd_interface->scsidevs->devs_present; i++)
	{
		SCSIAllocInstance( machine, wd_interface->scsidevs->devices[i].scsiClass, &devices[wd_interface->scsidevs->devices[i].scsiID], wd_interface->scsidevs->devices[i].diskregion );
	}
...
void wd33c93_exit( const struct WD33C93interface *wd_interface )
{
	int i;

	for (i = 0; i < wd_interface->scsidevs->devs_present; i++)
	{
		SCSIDeleteInstance( devices[wd_interface->scsidevs->devices[i].scsiID] );
	}
}
