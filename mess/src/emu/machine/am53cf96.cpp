...
void am53cf96_init( running_machine *machine, const struct AM53CF96interface *am_interface )
{
	int i;
	...
	for (i = 0; i < am_interface->scsidevs->devs_present; i++)
	{
		SCSIAllocInstance( machine, am_interface->scsidevs->devices[i].scsiClass, &devices[am_interface->scsidevs->devices[i].scsiID], am_interface->scsidevs->devices[i].diskregion );
	}
...
void am53cf96_exit( const struct AM53CF96interface *am_interface )
{
	int i;

	for (i = 0; i < am_interface->scsidevs->devs_present; i++)
	{
		SCSIDeleteInstance( devices[am_interface->scsidevs->devices[i].scsiID] );
	}
}
