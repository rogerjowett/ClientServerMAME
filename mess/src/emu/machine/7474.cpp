...
struct _ttl7474_state
{
 ...
 ttl7474_state *get_safe_token_7474(running_device *device)
{
	assert(device != NULL);
...
{
	ttl7474_state *dev_state = get_safe_token_7474(device);
...
static DEVICE_START( ttl7474 )
{
	...
	ttl7474_state *state = get_safe_token_7474(device);
