...
***************************************************************************/
 ...
 adc0831_state *get_safe_token_adc083x( running_device *device )
{
	assert( device != NULL );
 ...
 const adc083x_interface *get_interface_adc083x( running_device *device )
{
	assert( device != NULL );
	assert( ( device->type() == ADC0831 ) || ( device->type() == ADC0832 ) || ( device->type() == ADC0834 ) || ( device->type() == ADC0838 ) );
...
{
	adc0831_state *adc083x = get_safe_token_adc083x( device );
...
{
	adc0831_state *adc083x = get_safe_token_adc083x( device );
	const adc083x_interface *intf = get_interface_adc083x( device );
