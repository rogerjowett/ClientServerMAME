...
-------------------------------------------------*/
 ...
 laserdisc_state *get_safe_token_ldcore(running_device *device)
{
	assert(device != NULL);
...
static void vblank_state_changed(screen_device &screen, void *param, bool vblank_state)
{
	...
	laserdisc_state *ld = get_safe_token_ldcore(device);
	ldcore_data *ldcore = ld->core;
...
{
	return get_safe_token_ldcore(device);
}
...
static const ldplayer_interface *get_interface_ldcore(const device_config *devconfig)
{
	if (devconfig == NULL)
		return NULL;
...
DEVICE_GET_INFO( laserdisc )
{
	...
	switch (state)
	{
		...
		case DEVINFO_PTR_ROM_REGION:			intf = get_interface_ldcore(device); info->romregion = (intf != NULL) ? intf->romregion : NULL; break;
		case DEVINFO_PTR_MACHINE_CONFIG:		intf = get_interface_ldcore(device); info->machine_config = (intf != NULL) ? intf->machine_config : NULL; break;
		...
		/* --- the following bits of info are returned as NULL-terminated strings --- */
		case DEVINFO_STR_NAME:					intf = get_interface_ldcore(device); strcpy(info->s, (intf != NULL) ? intf->name : "Unknown Laserdisc Player");	break;
