 ...
 *************************************/
 ...
 s2636_state *get_safe_token_s2636( running_device *device )
{
	assert(device != NULL);
 ...
 const s2636_interface *get_interface_s2636( running_device *device )
{
	assert(device != NULL);
	assert((device->type() == S2636));
...
{
	s2636_state *s2636 = get_safe_token_s2636(device);
	int checksum = 0;
...
{
	s2636_state *s2636 = get_safe_token_s2636(device);
	const s2636_interface *intf = get_interface_s2636(device);
